...
const categories = ["Packing", "Invoice", "Stock"];

const wasteReportDefinitions = [
  {
    id: "waste-packing-details",
    name: "Waste Packing Details Report",
    category: "Packing",
    status: "enabled",
    parameters: [
    ...
    handler: "packingDetails",
  },
  {
    id: "waste-type-wise",
    name: "Waste Type Wise Packing Details",
    category: "Packing",
    status: "enabled",
    parameters: [
      { key: "fromDate", label: "From Date", type: "date", required: true },
  ...
  {
    id: "waste-invoice-no-wise",
    name: "Waste Invoice No Wise",
    category: "Invoice",
    status: "enabled",
    parameters: [
      { key: "fromDate", label: "From Date", type: "date", required: true },
    ...
    handler: "invoiceNoWise",
  },
  {
    id: "waste-stock-abstract",
    name: "Waste Stock Abstract Report",
    category: "Stock",
    status: "enabled",
    parameters: [
      { key: "fromDate", label: "From Date", type: "date", required: true },
    ...
    handler: "stockAbstract",
  },
  {
    id: "waste-closing-stock-value",
    name: "Waste Closing Stock Value Report",
    category: "Stock",
    status: "enabled",
    parameters: [
      { key: "fromDate", label: "From Date", type: "date", required: true },
    ...
    handler: "closingStockValue",
  },
  {
    id: "waste-stock-yearly",
    name: "Waste Stock Yearly Abstract Report",
    category: "Stock",
    status: "enabled",
    parameters: [
      { key: "fromDate", label: "From Date", type: "date", required: true },
    ...
    handler: "yearlyStockAbstract",
  },
];

const WasteReportsPage = () => {
  ...
  useEffect(() => {
    const packingReports = wasteReportDefinitions.filter((r) => r.category === "Packing");
    setSelected(packingReports[0] || null);
  }, []);

  const reports = useMemo(
    () => wasteReportDefinitions.filter((report) => report.category === category),
    [category]
  );
  ...
  const generate = async (event) => {
    ...
    try {
      ...
      switch (selected.handler) {
        case "packingDetails":
          report = await wastePackingService.getPackingDetailsReport(fromDate, toDate);
          report = formatPackingDetailsReport(report, fromDate, toDate);
          break;
        case "typeWise":
          report = await wastePackingService.getTypeWisePackingReport(fromDate, toDate);
          report = formatTypeWiseReport(report, fromDate, toDate);
          break;
        case "invoiceNoWise":
          report = await invoiceService.getInvoiceNoWiseReport(fromDate, toDate);
          report = formatInvoiceNoWiseReport(report, fromDate, toDate);
          break;
        case "stockAbstract":
          report = await wastePackingService.getStockAbstractReport(fromDate, toDate);
          report = formatStockAbstractReport(report, fromDate, toDate);
          break;
        case "closingStockValue":
          report = await wastePackingService.getClosingStockValueReport(fromDate, toDate);
          report = formatClosingStockValueReport(report, fromDate, toDate);
          break;
        case "yearlyStockAbstract":
          report = await wastePackingService.getYearlyStockAbstractReport(fromDate, toDate);
          report = formatYearlyStockAbstractReport(report, fromDate, toDate);
          break;
        default:
  ...
  // Format functions
  const formatPackingDetailsReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      ...
      baleNo: row.baleNo,
      grossWeight: parseFloat(row.grossWeight || 0).toFixed(2),
      tareWeight: parseFloat(row.tareWeight || 0).toFixed(2),
      netWeight: parseFloat(row.netWeight || 0).toFixed(2),
    })) || [];

    return {
      reportTitle: "Waste Packing Details Report",
      fromDate,
      toDate,
      columns: [
        ...
        { key: "packingType", label: "Packing Type", align: "left" },
        { key: "baleNo", label: "Bale No", align: "left" },
        { key: "grossWeight", label: "Gross Wt", align: "right" },
        { key: "tareWeight", label: "Tare Wt", align: "right" },
        { key: "netWeight", label: "Net Wt", align: "right" },
      ],
      rows,
      totals: {
        netWeight: data.summary?.totalWeight?.toFixed(2) || "0.00",
      },
    };
  };

  const formatTypeWiseReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      date: row.date,
      lotNo: row.lotNo,
      packingType: row.packingType,
    ...
    return {
      reportTitle: "Waste Type Wise Packing Details",
      fromDate,
      toDate,
      columns: [
        { key: "date", label: "Date", align: "left" },
  ...
  const formatInvoiceNoWiseReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      invoiceNo: row.invoiceNo,
      date: row.date,
      partyName: row.partyName,
      totalBales: row.totalBales,
      totalWeight: parseFloat(row.totalWeight || 0).toFixed(2),
      assessableValue: parseFloat(row.assessableValue || 0).toFixed(2),
      invoiceValue: parseFloat(row.invoiceValue || 0).toFixed(2),
    })) || [];

    return {
      reportTitle: "Waste Invoice No Wise Report",
      fromDate,
      toDate,
      columns: [
        ...
        { key: "totalBales", label: "Total Bales", align: "right" },
        { key: "totalWeight", label: "Total Weight (Kg)", align: "right" },
        { key: "assessableValue", label: "Assessable Value", align: "right" },
        { key: "invoiceValue", label: "Invoice Value", align: "right" },
      ],
      rows,
      totals: {
        totalBales: data.summary?.totalBales || 0,
        totalWeight: data.summary?.totalWeight?.toFixed(2) || "0.00",
        assessableValue: data.summary?.totalAssessableValue?.toFixed(2) || "0.00",
        invoiceValue: data.summary?.totalInvoiceValue?.toFixed(2) || "0.00",
      },
    };
  };

  const formatStockAbstractReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      code: row.code,
      wasteName: row.wasteName,
    ...
    return {
      reportTitle: "Waste Stock Abstract Report",
      fromDate,
      toDate,
      columns: [
  ...
  const formatClosingStockValueReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      code: row.code,
      wasteName: row.wasteName,
      closing: row.closing,
    ...
    return {
      reportTitle: "Waste Closing Stock Value Report",
      fromDate,
      toDate,
      columns: [
        { key: "code", label: "Code", align: "left" },
  ...
  const formatYearlyStockAbstractReport = (data, fromDate, toDate) => {
    const rows = data.data?.map((row) => ({
      month: row.month,
      opening: row.opening || 0,
      production: row.production || 0,
    ...
    return {
      reportTitle: "Waste Stock Yearly Abstract Report",
      fromDate,
      toDate,
      columns: [
      ...
      rows,
      totals: {
        opening: data.summary?.opening || 0,
        production: data.summary?.production || 0,
  ...
  // When viewing a report, show full screen layout
  if (reportData) {
    return (
  ...
  // Default view - Show filters and reports list
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <ToastContainer position="top-right" autoClose={3000} />
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-2xl font-bold text-slate-900">Waste Cotton Reports</h1>
          ...
          <aside className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm h-fit">
            {categories.map((item) => (
              <button
                ...
                onClick={() => {
                  setCategory(item);
                  const firstReport = wasteReportDefinitions.find((r) => r.category === item);
                  setSelected(firstReport || null);
                  setValues({});
                  setReportData(null);
                  setCurrentPage(1);
                }}
                className={`mb-1 w-full rounded-md px-4 py-3 text-left text-sm font-semibold transition ${
                  category === item
                    ? "bg-slate-900 text-white"
